/*
 * alive.c: a simple fault detection system for doRedis/future.redis.
 * Fault tolerance works by assigning two keys to each in process task: a
 * permanent key created when the task starts, and an ephemeral key kept alive
 * by this thread while the task is running.  If the thread defined in this
 * code stops, for example if R crashes, then the ephemeral key will expire and
 * the coordinating R process can detect the resulting keypair imbalance and
 * re-submit the corresponding task.
 */
#ifdef Win32
#include <winsock2.h>
#include <ws2tcpip.h>
...
SEXP
setAlive (SEXP PORT, SEXP HOST, SEXP KEY, SEXP AUTH)
{
  if (go > 0)
    return (R_NilValue);
#ifdef Win32
...
static const R_CallMethodDef CallEntries[] = {
  {"setAlive", (DL_FUNC) &setAlive, 4},
  {"delAlive", (DL_FUNC) &delAlive, 0},
  {NULL, NULL, 0}
};
...
#endif
void
R_init_future_redis (DllInfo * dll)
{
